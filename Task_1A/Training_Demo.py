...
def training_function(model, number_of_epochs, tensors_and_iterable_training_data, loss_function, optimizer):
    # Initialize lists to store training losses and validation accuracies
    train_losses = []
    val_accuracies = []
    ...
    for epoch in range(number_of_epochs):
        ...
        val_accuracy = validation_function(model, tensors_and_iterable_training_data)
        val_accuracies.append(val_accuracy)

    # Plot and save the accuracy graph
    plt.plot(range(1, number_of_epochs + 1), val_accuracies, label='Validation Accuracy')
    plt.xlabel('Epoch')
    plt.ylabel('Accuracy')
